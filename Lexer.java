...
enum TokenType {
    ...
    FOR, TO, STEP   // <-- new tokens for for-loop
}
...
class Lexer {
    ...
    // ── Keyword table ──────────────────────────────────────────────────────────
    // Grammar reference: see GRAMMAR.md §7 Keywords
    private static final Map<String, TokenType> KEYWORDS = Map.of(
        "পূর্ণসংখ্যা", TokenType.INT_TYPE,
        "বুলিয়ান",     TokenType.BOOL_TYPE,
        "সত্য",        TokenType.TRUE,
        "মিথ্যা",      TokenType.FALSE,
        "যদি",         TokenType.IF,
        "নাহলে",       TokenType.ELSE,
        "দেখান",       TokenType.PRINT,
        "প্রতিটি",     TokenType.FOR,   // for
        "থেকে",        TokenType.TO,    // to  (range separator)
        "ধাপ",         TokenType.STEP   // step
    );
    ...
    // ── Operator scanner ───────────────────────────────────────────────────────
    // Grammar reference: see GRAMMAR.md §5 Operators
    private Token scanOperator(int l, int c) {
        ...
        switch (ch) {
            ...
            default:  throw new RuntimeException("Unknown char: " + ch + " at line " + l + ", col " + c);
        }
    }

    // ── Number scanner ─────────────────────────────────────────────────────────
    // Grammar reference: see GRAMMAR.md §6 Terminals – NUMBER
    ...
    // ── Word / keyword scanner ─────────────────────────────────────────────────
    // Grammar reference: see GRAMMAR.md §6 Terminals – IDENT / §7 Keywords
    private Token scanWord(int l, int c) {
        ...
        if (type == null) return new Token(TokenType.IDENT, word, l, c);
